...
npf-compare local:Multicast --test ./scaling.npf --cluster\
    ...
    --cluster-autosave --variables DEBUG=0 "TIMER={0,5}"\
    "NB_CLIENTS={1,25,50,100,150,200,250}"\
    "PURE_UDP={0}"\
    CPUSUM=0 "PACING=0" "ACK_ELICIT={0}" "NTHREADS={16}"\
    "INTHROUGHPUT={150}"\
    "MULTICAST={1}"\
    SENDMMSG=0\
    --show-files --show-cmd --single-output results-fcquic.csv --config n_runs=1 --variables V=3 --force-retest