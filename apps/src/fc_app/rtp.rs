...
impl RtpServer {
    ...
    #[inline]
    pub fn handle_new_rtp_frame(
        &mut self, buf_type: BufType, next_stream_id: u64,
    ) {
        ...
        if n >= self.stop_msg.len() &&
            n.abs_diff(self.stop_msg.len()) <= 1 &&
            &buf[..self.stop_msg.len()] == &self.stop_msg
        {
            // STOP RTP message.
