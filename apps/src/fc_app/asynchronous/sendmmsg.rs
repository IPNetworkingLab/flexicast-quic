...
impl SendMMsg {
    ...
    pub async fn run(&mut self) -> Result<()> {
        loop {
            ...
            match msg {
                MsgSmsg::Packet(mut packet) => unsafe {
...
unsafe fn my_sendmmsg(
    socket: &tokio::net::UdpSocket, buf: &mut [u8],
    sock_addrs: &[std::net::SocketAddr],
) -> io::Result<usize> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        for i in 0..n {
            ...
            msgs[i].msg_hdr.msg_iov =
                &mut iovecs[i] as *mut _ as *mut libc::iovec;
            msgs[i].msg_hdr.msg_iovlen = 1;
            match &sock_addrs[i] {
                SocketAddr::V4(v4) => {
                    addresses[i].sin_family = libc::AF_INET as u16;
                    addresses[i].sin_port = u16::to_be(v4.port());
                    addresses[i].sin_addr = in_addr {
                        s_addr: u32::to_be(v4.ip().to_owned().into()),
                    };

                    msgs[i].msg_hdr.msg_name =
                        &mut addresses[i] as *mut _ as *mut libc::c_void;
                    msgs[i].msg_hdr.msg_namelen =
                        std::mem::size_of::<libc::sockaddr_in>() as u32;

                    debug!(
                        "address: {:?} and {:?} and {:?}",
                        addresses[i].sin_addr,
        ...
        let _retval =
            sendmmsg(sock_fd, &mut msgs[0] as *mut libc::mmsghdr, n as u32, 0);
